...
The core value proposition of using a CRDT like Automerge is the mathematical guarantee of data 
consistency without central arbitration. The system ensures three key properties:

\begin{itemize}
    \item \textbf{Commutativity}: The order in which concurrent operations are applied does not 
    affect the final document state. \texttt{apply(A) then apply(B)} yields the same result as 
    \texttt{apply(B) then apply(A)}.
    \item \textbf{Causal Ordering}: If operation B depends on operation A (e.g., a reply to a 
    comment), the system ensures A is applied before B across all replicas.
    \item \textbf{Eventual Convergence}: Provided all clients eventually receive all messages, 
    every client will see the exact same whiteboard state.
\end{itemize}

\paragraph{Concrete Examples:}
\begin{enumerate}
    \item \textbf{Concurrent Drawing}: If User A and User B draw a stroke simultaneously, Automerge 
    treats these as two independent insertions into the \texttt{strokes} list. Since CRDT lists allow 
    concurrent insertions, both strokes will be preserved and rendered on the canvas. The relative 
    order of these strokes is determined deterministically (typically by peer ID), ensuring both 
    users see the strokes in the same Z-order.
    
    \item \textbf{Clear vs. Draw}: Consider a scenario where User A clears the whiteboard (removing 
    all strokes) while User B simultaneously draws a new stroke. Automerge resolves this by executing 
    the removal of the known set of strokes and the insertion of the new stroke. If User B's stroke 
    is logically new and not part of the set User A removed, the result is typically a cleared board 
    containing only User B's new stroke. This behavior provides a predictable and non-destructive 
