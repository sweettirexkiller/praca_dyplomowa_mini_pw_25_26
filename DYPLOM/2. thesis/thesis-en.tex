...
state and UX interaction methods, \texttt{backend\_api.rs} -- a module of abstraction which defines connection points 
 of canvas/CRDT and networking, becasue what user sees on the screen is not just a matrix of pixeels from 
 persistent storage, but a result of rendering of CRDT document.
 
 \texttt{automerge\_backend.rs} -- a module which defines the CRDT document and methods of interaction with it. It is 
 a implementation  of \texttt{backend\_api.rs} with a little bit of extra methods.
