...
\begin{itemize}
	...
	\item \textbf{Synchronization} -- The \texttt{peer\_connected} and \texttt{peer\_disconnected} methods
		  ...
		  reflecting any changes. Conversely, \texttt{generate\_sync\_message} produces an outgoing
		  binary message destined for a specific peer, returning \texttt{None} when there is nothing
		  to send.

	\item \textbf{Persistence} -- The \texttt{save} method serializes the entire document state into
		  a byte vector, while \texttt{load} restores the document from previously saved bytes. These
		  methods enable explicit file export and import without requiring any default persistence.
\end{itemize}

The communication data types are also defined in this module. \texttt{Point} represents a 2D coordinate,
...
By keeping these types in a shared module, both the UI and any backend implementation can reference
